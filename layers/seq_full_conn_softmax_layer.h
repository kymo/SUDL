...
#ifndef SEQ_FULL_CONN_SOFTMAX_LAYER_
#define SEQ_FULL_CONN_SOFTMAX_LAYER_

#include "layer.h"

namespace sub_dl {

class SeqFullConnSoftmaxLayer : public Layer {
...
public:
    ...
    SeqFullConnSoftmaxLayer(int input_dim, int output_dim) {
        _type = SEQ_FULL;
    ...
    void _forward(Layer* pre_layer) {
        ...
        for (int i = 0; i < _seq_len; i++) {
             matrix_double val = exp_m(pre_layer->_data[i] * _seq_full_weights + _seq_full_bias);
    ...
    void _backward(Layer* nxt_layer) {
        ...
        for (int i = _seq_len - 1; i >= 0; i--) {
               matrix_double error = nxt_layer->_errors[i] - _data[i] * nxt_layer->_errors[i].sum();
               _errors.push_back(error);
            _delta_seq_full_weights.add(_pre_layer->_data[i]._T() * error);
            _delta_seq_full_bias.add(error);
          }
