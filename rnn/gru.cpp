...
GRU::GRU(int feature_dim, int hidden_dim, int output_dim) : 
    ...
    _delta_ug_input_weights.assign_val();
    _delta_ug_hidden_weights.assign_val();
    _delta_ug_bias.assign_val();
    
    _rg_input_weights.assign_val();
    _rg_hidden_weights.assign_val();
    _rg_bias.assign_val();

    _delta_rg_input_weights.assign_val();
    _delta_rg_hidden_weights.assign_val();
    _delta_rg_bias.assign_val();

    _newh_input_weights.assign_val();
    _newh_hidden_weights.assign_val();
    _newh_bias.assign_val();

    _delta_newh_input_weights.assign_val();
    _delta_newh_hidden_weights.assign_val();
    _delta_newh_bias.assign_val();
...
void GRU::_forward(const matrix_double& feature) {
    ...
    for (int t = 0; t < seq_len; t++) {
        ...
        matrix_double rg_value = sigmoid_m(xt * _rg_input_weights
            ...
            + _rg_bias);
        matrix_double newh_value = sigmoid_m(xt * _newh_input_weights
            + rg_value.dot_mul(pre_hidden_vals * _newh_hidden_weights)
            + _newh_bias);
        matrix_double hidden_value = rg_value.dot_mul(newh_value) - (ug_value - 1).dot_mul(pre_hidden_vals) ;
    ...
    for (int t = seq_len - 1; t >= 0; t--) {
        ...
        if (t != seq_len - 1) {
            mid_hidden_error = mid_hidden_error 
                ...
                + nxt_hidden_error.dot_mul(_ug_values._R(t + 1) - 1) * -1;
        }
        matrix_double newh_error = mid_hidden_error
            .dot_mul(_ug_values._R(t))
            .dot_mul(tanh_m_diff(_newh_values._R(t)));

        matrix_double ug_error = output_error * _hidden_output_weights._T();
        if (t > 0) {
        ...
        if (t > 0) {
            ...
            _delta_newh_hidden_weights.add(_hidden_values._R(t - 1)._T() * (ug_error.dot_mul(_rg_values._R(t))));
        }
